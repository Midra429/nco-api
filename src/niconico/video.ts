...
export const video = async (
  ...
  if (/^[a-z]{2}\d+$/.test(contentId)) {
    const url = new URL(contentId, guest ? API_BASE_URL_GUEST : API_BASE_URL)
    ...
    try {
      ...
      if (!isResponseOk(json)) {
        throw new Error(
          `${json.meta.status} ${json.meta.errorCode}: ${json.data.reasonCode}`
        )
      }
...
export const multipleVideo = (
  ...
  return Promise.all(contentIds.map((contentId) => video(contentId, guest)))
}
